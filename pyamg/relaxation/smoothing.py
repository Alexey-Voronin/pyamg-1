...
from ..krylov import fgmres, gmres, cgne, cgnr, cg
...
KRYLOV_RELAXATION = ['cg', 'cgne', 'cgnr', 'fgmres', 'gmres',
                     'fgmres_smoother',
                     ]
...
def setup_fgmres_smoother(lvl,
    ...
    fn0 = _setup_call('fgmres')
    smoother = fn0(lvl, tol=tol, maxiter=iterations, restrt=restrt,
                   M=M, callback=callback, residuals=residuals)

    update_wrapper(smoother, fgmres)  # set __name__
    return smoother

def setup_fgmres(lvl, tol=1e-12, maxiter=DEFAULT_NITER, restrt=None, M=None, callback=None,
                residuals=None):
    """Set up GMRES smoothing."""
    def smoother(A, x, b):
        x[:] = fgmres(A, b, x0=x, tol=tol, maxiter=maxiter, restrt=restrt, M=M,
...
def _setup_call(fn):
    ...
    setup_register = {
        ...
        'cgnr':                   setup_cgnr,
        'none':                   setup_none,
        'fgmres_smoother':        setup_fgmres_smoother
    }
